...
header() {
    ...
    echo "   |   v67.1: hotfix runtime G3 (cnt multilinea + sed con backticks)      |"
    echo "   ==========================================================================="
    echo -e "${NC}"
...
# [v67-G2] Funcion auxiliar: elegir helper canonico por nombre con scoring
# robusto. Algoritmo:
...
verificar_flujo_caja() {
    ...
    # [v67-G3] Investigado en VPS: ningun helper usa throw new Error('UPPERCASE').
...
' 2>/dev/null)
    ...
    if [ -d "${UTILS_DIR:-}" ] && verificar_bd 2>/dev/null; then
        ...
        for tbl in $tablas_principales; do
            # [v67-G2 v2] Usar scoring helper con normalizacion.
            local hp_canon=""
            local todos_helpers; todos_helpers=$(ls "$UTILS_DIR"/*.helper.js 2>/dev/null | tr '\n' ' ')
            hp_canon=$(_elegir_helper_canonico "$tbl" "$todos_helpers")
            [ -z "$hp_canon" ] && continue
            # Otros que escriben
            local invasores
...
ejecutar_todo_detalle() {
    ...
    for entry in "${cli_funcs[@]}"; do
        ...
        # [v67-G1] Dedup defensivo: si la funcion escribio su propio header
    ...
    local FM="${OUTPUT_DIR}/MODULOS_${TS_M}.md"
    {
        ...
        echo "## Generado: $(date '+%Y-%m-%d %H:%M') | Toolkit v${VERSION}"
        echo ""
        # [v67-G1] Filtro ANSI literal (probado: ESC se pierde antes de llegar)
...
mostrar_ayuda() {
    cat << 'HELP'
ERP LAGO - Toolkit IA v67.1  (Solution Architect Mode)
