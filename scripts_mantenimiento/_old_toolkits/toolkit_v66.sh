...
#
# Uso: ./toolkit_v60.sh [comando] [opciones]
# =======================================================================================
...
header() {
    ...
    echo "   |   v66: arquitecto - helper canonico real + fragmentacion + drift     |"
    echo "   ==========================================================================="
    echo -e "${NC}"
    echo -e "  Proyecto: ${CYAN}$PROJECT_ROOT${NC}"
    echo -e "  Salida:   ${CYAN}$OUTPUT_DIR${NC}"
    # [v63] mostrar path real del script: ayuda a debuggear si conviven varias versiones
...
_run_sql() {
    # [v63] timeout 10s: si una query queda bloqueada (lock, query lenta),
...
_ejecutar_y_guardar() {
    ...
    "$funcion" "$OUT" "$@"
    # [v66-F2] Dedup robusto: si las primeras 2 lineas no-vacias empiezan
...
_ejecutar_capturando_stdout() {
    ...
    local OUT="${OUTPUT_DIR}/${nombre}_${TS_LOCAL}.md"
    {
        ...
        echo "## Generado: $(date '+%Y-%m-%d %H:%M') | Toolkit v${VERSION}"
        echo ""
        # [v66-F1] Probado en sandbox: el archivo NO contiene byte ESC (0x1B)
...
detectar_estado_multiempresa() {
    ...
    total_tablas=$(_run_sql \
        "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema='public' AND table_type='BASE TABLE' AND table_name NOT LIKE '\_backup\_%'")
        ...
        if [ -n "$tablas_empresa" ]; then
            while IFS= read -r tbl; do
                ...
                if [ "$selects_sin" -gt 0 ]; then
                    ...
                    hay_problemas=1
                    # [v66-F5] Mostrar archivo:linea + snippet de las primeras 3 queries
                    local muestras
...
_get_tablas_enterprise() {
    # [v59] Filtra _backup_* — no son tablas vivas.
...
verificar_auditoria_multiempresa() {
    ...
    # ═══ CHECK 5: Firmas helpers (v59 — stateless-aware) ═══
    ...
    # [v59] Excluir tablas de config del set enterprise.
    local ent_alt=""
    ...
    while IFS= read -r hfile; do
        ...
        while IFS= read -r fn; do
            ...
            # [v59] Skip funciones puras por patrón idiomatic de nombre.
            case "$fn" in
...
verificar_constraints_triggers() {
    ...
    _out+=$'\n'
    # [v60] Check especifico: usuario_configuracion UNIQUE debe ser multi-empresa
...
# =======================================================================================
# [v59-C6] VERIFICAR LAGO.AR (sitio servido desde VPS, modelo nuevo post-migracion)
# =======================================================================================
...
verificar_integridad_referencial() {
    ...
    # [v59] Solo FKs con propagación (CASCADE/RESTRICT/SET NULL/SET DEFAULT) — son las que
...
detectar_endpoints_huerfanos() {
    ...
    while IFS= read -r route_file; do
        ...
        # [v59] Extraer aliases reales del routes file (const X = require('./xx.controller'))
...
EOF
    ...
    # [v66-F3] Helper canonico = (a) si hay un helper con nombre tabla.helper.js
    ...
    if [ -n "${UTILS_DIR:-}" ] && [ -d "$UTILS_DIR" ]; then
        ...
        # (a) Helper cuyo basename matchea la tabla
        local tabla_singular="${tabla%s}"
        local hp_match
        for hp in $helpers_que_escriben; do
            hp_match=$(basename "$hp" .helper.js)
            # Match exacto de tabla (e.g. movimientos_caja.helper.js para tabla movimientos_caja)
            if [ "$hp_match" = "$tabla" ] || [ "$hp_match" = "$tabla_singular" ]; then
                canonico_por_nombre="$hp"
                break
            fi
            # Match parcial: helper contiene "tabla" o vice versa
            case "$tabla" in
                "${hp_match}"*|*"${hp_match}") canonico_por_nombre="$hp"; break ;;
            esac
        done
    ...
    # [v66-F4] Antes el ALERTA salia siempre que total_ops>3, sin distinguir
...
generar_informe_salud() {
    ...
    # [v59] §2 eliminada — duplicaba Check 1 de auditoria-me (§8) con peor análisis.
    ...
    if [ -d "$CONTROLLERS_DIR" ] && [ -n "${UTILS_DIR:-}" ] && [ -d "$UTILS_DIR" ]; then
        ...
        while IFS= read -r update_line; do
            ...
            [ -z "$ut_tabla" ] && continue
            # [v59] Verificar si la tabla tiene helper DUEÑO (hace SQL sobre la tabla)
            # Antes: grep -rwl matcheaba menciones en comentarios/logs → asociaciones falsas.
            local ut_helper=""
            ut_helper=$(grep -rlE "INSERT INTO ${ut_tabla}\b|UPDATE ${ut_tabla}[[:space:]]+SET|DELETE FROM ${ut_tabla}\b|FROM ${ut_tabla}\b" "$UTILS_DIR" --include="*.helper.js" 2>/dev/null | head -1 || true)
            if [ -n "$ut_helper" ]; then
                local ut_file=""
...
EOF
    ...
    for ctrl in "$CONTROLLERS_DIR"/*.controller.js; do
        [ -f "$ctrl" ] || continue
        local nombre nexp1 nexp2 nexp
        nombre=$(basename "$ctrl")
        # [v59] Contar AMBOS patrones: exports.X y module.exports = { X, Y }
        nexp1=$(grep -cE "^exports\." "$ctrl" 2>/dev/null); nexp1=${nexp1:-0}
        # Para module.exports = { ... } contar llaves tipo `  nombre,` dentro del bloque
        nexp2=$(awk '/module\.exports[[:space:]]*=[[:space:]]*\{/,/^\};?$/' "$ctrl" 2>/dev/null \
            | grep -cE "^[[:space:]]+[a-zA-Z_][a-zA-Z0-9_]*[[:space:]]*[:,]" 2>/dev/null); nexp2=${nexp2:-0}
        nexp=$((nexp1 + nexp2))
    ...
    # [v59] RUTAS API — solo resumen por archivo. Detalle en PARTE 6 / Endpoints por módulo.
    ...
    for route in "$ROUTES_DIR"/*.routes.js; do
        ...
        rcount=$(grep -cE "router\.(get|post|put|delete)" "$route" 2>/dev/null); rcount=${rcount:-0}
        echo "$nombre: $rcount rutas" >> "$CONTEXT_FILE"
    done
    echo '```' >> "$CONTEXT_FILE"
    echo "" >> "$CONTEXT_FILE"
...
# =======================================================================================
# [v59-C5] DUMP DE CONFIG KEYS (configuraciones_empresa)
# =======================================================================================
...
CAJABLOCK
    ...
    # === [v60] Modulos documentados en docs/sesiones (referencia rapida) ===
    if [ -d "$PROJECT_ROOT/docs/sesiones" ]; then
        echo "" >> "$PROMPT_FILE"
        echo "---" >> "$PROMPT_FILE"
        echo "" >> "$PROMPT_FILE"
        echo "## DOCUMENTACION DE MODULOS DISPONIBLE (docs/sesiones/)" >> "$PROMPT_FILE"
        echo "" >> "$PROMPT_FILE"
        echo "Re-leer estos .md ANTES de tocar el modulo correspondiente:" >> "$PROMPT_FILE"
        echo "" >> "$PROMPT_FILE"
        local md
        for md in "$PROJECT_ROOT/docs/sesiones"/MODULO_*.md "$PROJECT_ROOT/docs/sesiones"/DOCUMENTACION_*.md; do
            [ -f "$md" ] && echo "- \`$(basename "$md")\`" >> "$PROMPT_FILE"
        done
        echo "" >> "$PROMPT_FILE"
    fi
...
# =======================================================================================
# [v60] FLUJO CAJA — Empresa -> Usuario -> Deposito -> Caja -> Turno -> Movimiento
# =======================================================================================
...
verificar_flujo_caja() {
    ...
    # ---- 8. CODIGOS DE ERROR DEL FLUJO (extraidos del codigo) ----
    _out+="#### Codigos de error del flujo (auto-detectados)"$'\n'
    if [ -d "${UTILS_DIR:-}" ]; then
        local errs
        # [v65-FIX] awk es mas robusto que sed para extraer la cadena entre comillas
        # [v66-F8] Filtrar a helpers del flujo de caja, no toda la app.
        # SKU, SOBRE_COSTO etc venian de productos.helper, no del flujo.
        errs=$(grep -rhE "throw new Error\([\"']" \
            "$UTILS_DIR/caja.helper.js" \
            "$UTILS_DIR/cobranza.helper.js" \
            "$UTILS_DIR/cc-clientes.helper.js" \
            "$UTILS_DIR/cc-proveedores.helper.js" \
            "$UTILS_DIR/pagos.helper.js" \
            "$UTILS_DIR/pagos-proveedores.helper.js" \
            "$UTILS_DIR/tesoreria.helper.js" \
            "$UTILS_DIR/borrador.helper.js" \
            2>/dev/null \
            | grep -oE "[\"'][A-Z][A-Z0-9_]+[\"']" \
            | tr -d "\"'" \
            | sort -u)
        if [ -n "$errs" ]; then
            while IFS= read -r e; do [ -n "$e" ] && _out+="  - $e"$'\n'; done <<< "$errs"
        fi
    fi
    _out+=""$'\n'
...
# =======================================================================================
# [v61] EXTRA — Lo que NO entra en el INFORME_COMPLETO
# =======================================================================================
...
' 2>/dev/null)
        if [ -z "$pm2_info" ]; then
            _out+="(proceso erplago no encontrado en pm2)"$'\n'
        else
            _out+="${pm2_info}"$'\n'
        fi
        _out+='```'$'\n'$'\n'
    ...
    if verificar_bd 2>/dev/null; then
        ...
        if [ "${sn:-0}" -gt 0 ] 2>/dev/null; then
            local sn_top
            # [v66-F6] Detectar columna real (descripcion no existe en productos)
            local prod_name_col
    ...
    if [ -d "${UTILS_DIR:-}" ]; then
        ...
        if [ "${total_h:-0}" -gt 0 ] && [ "${scope_h:-0}" -lt "${total_h}" ]; then
            local sin_scope
            sin_scope=$(comm -23 \
    ...
    if verificar_bd 2>/dev/null && [ -d "${PROJECT_ROOT}/src" ]; then
        ...
        keys_codigo=$(grep -rohE "['\"]([a-z][a-z_]+\\.[a-z][a-z._]+)['\"]" "${PROJECT_ROOT}/src" --include="*.js" 2>/dev/null \
        ...
        if [ -n "$keys_codigo" ]; then
            ...
            if [ -n "$faltantes" ]; then
                ...
                # Top 10
                echo "$faltantes" | head -10 | while IFS= read -r line; do echo "$line"; done >> /dev/null
    ...
    # Modulos documentados (referencia)
    if [ -d "${PROJECT_ROOT}/docs/sesiones" ]; then
        _out+="### Modulos con doc canonica (re-leer antes de tocar)"$'\n'
        local md
        for md in "${PROJECT_ROOT}/docs/sesiones"/MODULO_*.md "${PROJECT_ROOT}/docs/sesiones"/DOCUMENTACION_*.md; do
            [ -f "$md" ] && _out+="- \`$(basename "$md")\`"$'\n'
        done
        _out+=""$'\n'
    fi
    ...
    if [ -d "${UTILS_DIR:-}" ] && verificar_bd 2>/dev/null; then
        ...
        for tbl in $tablas_principales; do
            local tbl_singular="${tbl%s}"
            # Helper canonico por nombre
            local hp_canon=""
            for hp in "$UTILS_DIR"/*.helper.js; do
                local hp_name; hp_name=$(basename "$hp" .helper.js)
                if [ "$hp_name" = "$tbl" ] || [ "$hp_name" = "$tbl_singular" ]; then
                    hp_canon="$hp"; break
                fi
                case "$tbl" in
                    "${hp_name}"*|*"${hp_name}") hp_canon="$hp"; break ;;
                esac
            done
            [ -z "$hp_canon" ] && continue
            # Otros que escriben
            local invasores
            invasores=$(grep -rlE "INSERT INTO ${tbl}\b|UPDATE ${tbl}\b|DELETE FROM ${tbl}\b" "$UTILS_DIR" --include="*.helper.js" 2>/dev/null \
                | grep -v "^${hp_canon}\$" | xargs -n1 basename 2>/dev/null | sort -u)
            if [ -n "$invasores" ]; then
...
# =======================================================================================
# [v62] DETALLE COMPLETO — Todas las opciones CLI en una sola pasada
# =======================================================================================
...
# =======================================================================================
# [v63] SELF-DOC: documentacion interna auto-generada del toolkit
# =======================================================================================
generar_self_doc() {
...
HEAD_EOF
    ...
    while IFS=: read -r linea decl; do
        ...
        i=$((i + 1))
        # [v66-F9] Antes mostraba "Comentario previo" siempre vacio. Ahora
...
# =======================================================================================
# [v63] CLEANUP de versiones viejas del toolkit
# =======================================================================================
...
menu_principal() {
    # [v61] Menu simplificado: 5 opciones reales. Las opciones 6-20 del v60
    # siguen accesibles via CLI directa (./toolkit_v61.sh helpers, migracion,
    # flujo, etc.) — solo se sacan del menu visual para no inflar la pantalla.
    while true; do
        header
        echo -e "  ${CYAN}┌──────────────────────────────────────────────────────────┐${NC}"
...
mostrar_ayuda() {
    cat << 'HELP'
ERP LAGO - Toolkit IA v66.0  (Solution Architect Mode)

Uso: ./toolkit_v61.sh [comando]   |   sin args = menu interactivo
...
----- MENU VISUAL (6 opciones) -----------------------------------
  ...
  6  detalle   / cli-todo      v62: TODAS las opciones CLI en un archivo
                               (DETALLE_COMPLETO_*.md + cada una individual)

  NOTA v62: cada opcion individual deja su archivo en OUTPUT_DIR.
...
----- CLI DIRECTA (no aparecen en menu, igual funcionan) ----------
  ...
  self-doc         [v63] Genera TOOLKIT_INTERNAL.md (auto-doc del toolkit)
  changelog        [v63] Muestra changelog inline (v59 -> actual)
  cleanup-old-versions  [v63] Archiva toolkit_v*.sh viejos a _old_toolkits/
  help             Esta ayuda

----- EJEMPLOS ----------------------------------------------------
  ./toolkit_v61.sh                              # Menu (5 opciones)
  ./toolkit_v61.sh completo                     # Informe completo directo
  ./toolkit_v61.sh extra                        # Runtime + deudas + drift
  ./toolkit_v61.sh rastrear movimientos_caja    # Rastreo de tabla
  ./toolkit_v61.sh flujo                        # Jerarquia caja vivo
  ./toolkit_v61.sh helpers                      # Lista de helpers
HELP
}
