...
header() {
    ...
    echo "   |   v63: timeout SQL + self-doc + changelog + cleanup-old-versions     |"
    echo "   ==========================================================================="
    echo -e "${NC}"
...
## [v62] Wrapper genérico: garantiza que cada opcion individual produzca un archivo.
## Para funciones que aceptan $1=destino y escriben ahi.
## Uso: _ejecutar_y_guardar "HELPERS" detectar_helpers_existentes
_ejecutar_y_guardar() {
    local nombre="$1"
    local funcion="$2"
    shift 2
...
## [v62] Wrapper para funciones que solo imprimen a stdout (no aceptan $1=destino).
...
verificar_flujo_caja() {
    ...
    n_dep=$(_run_sql "SELECT COUNT(*) FROM depositos WHERE activo=true")
    n_caja=$(_run_sql "SELECT COUNT(*) FROM cajas WHERE activa=true")
    ...
    dep_sin_caja=$(_run_sql "
        ...
        FROM depositos d
        WHERE d.activo=true
        AND NOT EXISTS (SELECT 1 FROM cajas c WHERE c.id_deposito=d.id_deposito AND c.activa=true)
        ORDER BY d.codigo
    ")
    ...
    mov_origen=$(_run_sql "
        SELECT
            COALESCE(p.origen, '(sin origen)') || ': ' || COUNT(*) || ' filas'
        FROM movimientos_caja mc
        LEFT JOIN pagos p ON mc.id_recibo IS NOT NULL AND mc.id_recibo = p.id_recibo
        GROUP BY p.origen
        ORDER BY COUNT(*) DESC
    ")
    ...
    if [ -d "${UTILS_DIR:-}" ]; then
        local errs
        errs=$(grep -rohE "throw new Error\(['\"]([A-Z][A-Z0-9_]+)['\"]" "$UTILS_DIR" --include="*.helper.js" 2>/dev/null \
...
' 2>/dev/null)
    ...
    if verificar_bd 2>/dev/null; then
        ...
        if [ "${sn:-0}" -gt 0 ] 2>/dev/null; then
            local sn_top
            sn_top=$(_run_sql "
                SELECT i.id_producto || ' (' || COALESCE(p.descripcion, '?') || '): ' || i.stock_real
                FROM inventario i
    ...
    if verificar_bd 2>/dev/null && [ -d "${PROJECT_ROOT}/src" ]; then
        ...
        keys_codigo=$(grep -rohE "['\"]([a-z][a-z._]{4,})['\"]" "${PROJECT_ROOT}/src" --include="*.js" 2>/dev/null \
            | grep -E "(tesoreria\\.|cc\\.|despachos\\.|venta_rapida\\.|web\\.|afip|productos\\.alicuota|cliente|compras\\.|presupuestos\\.)" \
            | sed -E "s/.*['\"]([a-z][a-z._]+)['\"].*/\\1/" | sort -u)
        local n_codigo n_bd
...
| # | Funcion | Linea | Comentario previo (si existe) |
|---|---------|-------|-------------------------------|
HEAD_EOF
...
mostrar_ayuda() {
    cat << 'HELP'
ERP LAGO - Toolkit IA v63.0  (Solution Architect Mode)
