...
verificar_lago_ar() {
    ...
    if verificar_bd 2>/dev/null; then
        ...
        local cron_lago; cron_lago=$(crontab -l 2>/dev/null \
            | grep -vE '^[[:space:]]*#' \
            | grep -cE '(lago-cron-deploy|generar-catalogo-web|ftp.*lago\.ar|lftp.*publico/)' || echo 0)
        if [ "${cron_lago:-0}" -gt 0 ]; then
        ...
        local cron_comentado; cron_comentado=$(crontab -l 2>/dev/null \
            | grep -E '^[[:space:]]*#' \
            | grep -cE '(lago-cron-deploy|generar-catalogo-web|lago-deploy)' || echo 0)
        if [ "${cron_comentado:-0}" -gt 0 ]; then
...
EJEMPLOS:
  ./toolkit_v60.sh                              # Menu interactivo
  ./toolkit_v60.sh completo                     # Genera TODO directo
  ./toolkit_v60.sh rastrear movimientos_caja    # Rastreo especifico
  ./toolkit_v60.sh impresion                    # Mapeo 3 caminos de impresion
  ./toolkit_v60.sh lago                         # Estado lago.ar
HELP
}
