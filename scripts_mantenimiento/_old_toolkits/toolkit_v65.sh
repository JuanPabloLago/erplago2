...
header() {
    ...
    echo "   |   v65: 6 bugfixes (ANSI, header doble, flujo_caja queries, falsos+)  |"
    echo "   ==========================================================================="
    echo -e "${NC}"
...
_ejecutar_y_guardar() {
    ...
    "$funcion" "$OUT" "$@"
    # [v65] Deduplicar headers consecutivos si la funcion escribio uno propio
    # (defensa en profundidad por si alguien refactoriza mal una funcion)
    if [ -f "$OUT" ]; then
        awk '!/^# '"$nombre"'$/ || !seen++' "$OUT" > "${OUT}.dedup" && mv "${OUT}.dedup" "$OUT"
    fi
    echo ""
...
verificar_flujo_caja() {
    ...
    if [ -d "${UTILS_DIR:-}" ]; then
        ...
        errs=$(grep -rhE "throw new Error\([\"']" "$UTILS_DIR" --include="*.helper.js" 2>/dev/null \
            | grep -oE "[\"'][A-Z][A-Z0-9_]+[\"']" \
            | tr -d "\"'" \
            | sort -u)
        if [ -n "$errs" ]; then
...
mostrar_ayuda() {
    cat << 'HELP'
ERP LAGO - Toolkit IA v65.0  (Solution Architect Mode)
