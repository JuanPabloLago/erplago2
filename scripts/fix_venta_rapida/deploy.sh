...
cp frontend/js/venta-rapida-script.js "$BACKUP_DIR/"
...
# VR-1b
if grep -q "if (!borradorId) return;" frontend/js/venta-rapida-script.js | head -1 > /dev/null 2>&1; then
    echo "    ✅ VR-1b: Doble-check borradorId OK"
else
    # grep might not work well for this, check differently
    if python3 -c "c=open('frontend/js/venta-rapida-script.js').read(); print('OK' if 'setTimeout(async () => {\n        if (!borradorId) return;' in c else 'MISSING')" 2>/dev/null | grep -q OK; then
        echo "    ✅ VR-1b: Doble-check borradorId OK"
    else
...
# VR-5c
if grep -q "iva_porcentaje" frontend/js/venta-rapida-script.js; then
